...
// processDML actually processes each of the points once it has created the target database. The database
// databae can either be specified by the user, or it will be read from the import file.
//The saame goes for the retention policy it too can be overriden by specifying `rp` or else it too is set
// to whatever is spcfied in the DML.
